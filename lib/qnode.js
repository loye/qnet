            ...
            authAsServer.call(this, conn, function (err) {
                if (err) {
                    ...
                    establish.call(self, conn, function (err, link) {
                        if (!err && link) {
                            self_endpoint = self.node.profile.endpoint;
                            ...
                            if (link.profile.endpoint) {
                                if (!link.connections['client']) {
                                    ...
                                    self.broadcast('links.post', { links: [link.profile] }, link.profile.id);
                                }
                            }
                        }
                    });
    ...
    (function (proto) {
        ...
        proto.close = function () {
            ...
            this.command('close', function () {
                Object.keys(self.connections).forEach(function (key) { self.connections[key].close(); });
            });
        };
...
var Commandar = (function () {
    ...
    function execute(sender, command, data) {
        ...
        switch (command) {
            case 'links.get':
                sender.command('links.post', { links: links_get.call(this) });
                break;
            case 'links.post':
                if (data && Array.isArray(data.links)) {
                    links_post.call(this, data.links);
                    response_default(sender);
                } else {
                    response_error(sender, 'not links');
                }
                break;
            default:
                response_error(sender, 'command not supported');
                break;
        }
    ...
    function response_default(sender) {
        sender.command('response', { success: true, message: 'command executed successfully' });
    };

    function response_error(sender, message) {
        sender.command('response', { success: false, message: message });
    };
