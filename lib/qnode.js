    ...
    function execute(sender, command, data, callback) {
        ...
        switch (command) {
            case 'get.profile':
                ...
                break;
            case 'get.myendpoint':
                callback.call(null, null, { endpoint: sender.profile.endpoint });
                break;
            case 'get.links':
                sender.command('post.links', { links: links_get.call(this) });
                break;
...
function generateId() {
    return crypto.createHash('sha1').update(Math.abs(Math.random() * Math.random() * Date.now() | 0).toString()).digest('hex');
};
//var _ = (function () {
