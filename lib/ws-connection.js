...
var WSConnection = (function () {
    ...
    (function (proto) {
        ...
        proto.message = function (message) {
            if (this.connected) {
...
if (require.main === module) {
    var wsserver = listen({ port: 1337, path: '/' }, function (conn) {
        conn.on('message', function (m) {
            console.log(m);
        });
        conn.message({ message: 'welcome', date: new Date() }).message([1, 2, 3]);
        conn.pipe(conn);
    });
  
    var s2 = listen({ server: wsserver._server, path: '/hello' }, function (conn) {
        conn.message({ m: 'this is hello' });
        conn.on('message', function (m) { console.log(m); });
        conn.on('close', function () { console.log('closed'); });
    });

    console.log(wsserver.path, s2.path);
}
